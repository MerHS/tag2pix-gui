...
class RefineG(nn.Module):
    def __init__(self, input_dim=1, output_dim=3, input_size=256, layers=[5, 5, 5, 5]):
        super(RefineG, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.input_size = input_size
        self.cardinality = 16
        ...
        self.deconv1 = self._make_block(256 + 256, 4*256, layers[0])
    ...
    def forward(self, input, feature_tensor):
        out1 = self.conv1(input)
        out2 = self.conv2(out1)
        out3 = self.conv3(out2)
        out4 = self.conv4(out3)
...
class RefineD(nn.Module):
    def __init__(self, input_dim=3, output_dim=1, input_size=256):
        ...
        self.conv2 = self._make_block_1(32, 64)
        self.conv3 = self._make_block_1(64, 128)
        self.conv4 = self._make_block_2(128, 128)
        self.conv5 = self._make_block_1(128, 256)
        self.conv6 = self._make_block_2(256, 256)
        self.conv7 = self._make_block_1(256, 512)
        self.conv8 = self._make_block_2(512, 512)
        
        self.last_size = (input_size * input_size) // (2 ** 10) * 512
        self.adv_judge = nn.Sequential(
            nn.Linear(self.last_size, 1024),
            nn.LeakyReLU(0.2),
            nn.Linear(1024, 1),
            nn.Sigmoid()
        )
